...
(*      have c_neq_e:"c \<noteq>e" 
      ...
      proof
        ...
        next
          ...
          hence "ts',move ts ts'' v \<Turnstile> \<^bold>\<not> (c \<^bold>=e) \<^bold>\<and> \<^bold>\<langle> re(c) \<^bold>\<and> cl(e)\<^bold>\<rangle>" using neg by force
      ...
      next 
        ...
        next
          ...
          next
            ...
            hence "ts',move ts ts'' v \<Turnstile> \<^bold>\<not> (c \<^bold>=e) \<^bold>\<and> \<^bold>\<langle> cl(c) \<^bold>\<and> re(e)\<^bold>\<rangle>" using neg by blast
  ...
  (*    then have e_def:"ts'',move ts ts'' v \<Turnstile> \<^bold>\<not> safe(e)" by blast
      ...
      hence "ts',move ts ts'' v \<Turnstile> \<^bold>\<langle>re(c) \<^bold>\<and>re(e) \<^bold>\<rangle>" using e_def v'_def somewhere_leq by meson
...
proof -
  ...
  have ts_in_type: " ts_rep \<in> {ts :: ((cars\<Rightarrow>real) * (cars \<Rightarrow> lanes) * (cars\<Rightarrow>lanes) *((cars\<Rightarrow> (real\<Rightarrow> real)))*(cars\<Rightarrow>real)*(cars\<Rightarrow>real)) . 
  ...
  proof
    ...
    proof (rule notI)
      ...
      have "left (ext v') < 1 \<and> left (ext v') < 5" using rd rc v'_rel_c v'_rel_d sp_eq_c sp_eq_d by auto
      ...
      proof
        ...
        from less_len have "right (len v' ts c) \<le> right (len v ts c)" by (simp add: less_eq_real_int_def)
        with len_c have "right (len v' ts c) \<le> 1 " using Abs_real_int_inverse by auto
  ...
  (*    assume "ts,v\<Turnstile> \<^bold>\<not>(y \<^bold>= x) \<^bold>\<rightarrow> \<^bold>\<not> \<^bold>\<langle>re(y) \<^bold>\<and> re(x) \<^bold>\<rangle>"
      ...
      next
        ...
        proof
          ...
          proof (cases "x=d")
            assume "x=d"
            then show "ts,v \<Turnstile>\<^bold>\<not> \<^bold>\<langle>re(y) \<^bold>\<and> re(x)\<^bold>\<rangle>" using no_overlap_c_d y_eq_c by blast
          next
            assume x:"x \<noteq>d"
  ...
  proof -
    ...
    thus ?thesis by blast
  qed
  from safe and unsafe have "ts,v  \<Turnstile> \<^bold>\<forall>e. safe(e) \<^bold>\<and>  (\<^bold>\<exists> c. (@c  \<^bold>\<not>( \<^bold>\<forall>e. safe(e))))" by blast
  thus ?thesis by blast
qed
  
abbreviation DC'::"\<sigma>"
  where "DC' \<equiv> \<^bold>G ( \<^bold>\<forall> c d. \<^bold>\<not>(c \<^bold>= d) \<^bold>\<rightarrow> ( @d \<^bold>\<not>\<^bold>\<langle>re(c) \<^bold>\<and> re(d)\<^bold>\<rangle> ) \<^bold>\<rightarrow>   \<^bold>\<box>\<^bold>\<tau> @d \<^bold>\<not>\<^bold>\<langle>re(c) \<^bold>\<and> re(d)\<^bold>\<rangle>)"
    
    
abbreviation LC'::"\<sigma>"
  where "LC' \<equiv> \<^bold>G ( \<^bold>\<forall>d. (\<^bold>\<exists> c.  (@c (pcc c d)) \<^bold>\<or> (@d (pcc c d))) \<^bold>\<rightarrow> \<^bold>\<box>r(d) \<^bold>\<bottom> ) "
    
theorem safety:"\<Turnstile> (\<^bold>\<forall>e. @e ( safe e ) ) \<^bold>\<and> DC' \<^bold>\<and> LC' \<^bold>\<rightarrow> \<^bold>G (\<^bold>\<forall> e. @ e (safe e))"
proof (rule allI)+
  fix ts v
  show "ts,v \<Turnstile>( \<^bold>\<forall>e. @e (safe e ) ) \<^bold>\<and> DC' \<^bold>\<and> LC' \<^bold>\<rightarrow> \<^bold>G (\<^bold>\<forall> e. @e (safe e))"
  proof
    assume assm:"ts,v \<Turnstile> ( \<^bold>\<forall>e. @e (safe e) ) \<^bold>\<and> DC' \<^bold>\<and> LC'"
    from assm have init:"ts,v \<Turnstile> ( \<^bold>\<forall>e. @e (safe e) )" by simp
    from assm have DC :"ts,v \<Turnstile> DC'" by simp
    from assm have LC: "ts,v \<Turnstile> LC'" by simp
    ...
    proof (rule impI [THEN allI ]) 
      ...
      proof (induct ts\<equiv>"ts" ts'\<equiv>ts' arbitrary:ts'  rule:abstract.induct )
        ...
        thus ?case using init by fastforce
      next
        case (evolve ts' ts'' )
        have local_DC:"ts',move ts ts' v \<Turnstile> \<^bold>\<forall> c d. \<^bold>\<not>(c \<^bold>= d) \<^bold>\<rightarrow> (@d \<^bold>\<not>\<^bold>\<langle>re(c) \<^bold>\<and> re(d)\<^bold>\<rangle> ) \<^bold>\<rightarrow> ( \<^bold>\<box>\<^bold>\<tau> @d \<^bold>\<not>\<^bold>\<langle>re(c) \<^bold>\<and> re(d)\<^bold>\<rangle>)"
          using evolve.hyps DC by simp
        show ?case 
        proof (rule ccontr)
          assume "\<not> (ts'',move ts ts'' v \<Turnstile> (\<^bold>\<forall> e. @e (safe e)))"
          hence "ts'',move ts ts'' v \<Turnstile>\<^bold>\<not>(\<^bold>\<forall>e. @e (safe e))" by simp
          hence "ts'',move ts ts'' v \<Turnstile>\<^bold>\<exists> e. \<^bold>\<not>( @e (safe e))" by simp
          from this obtain e where e_def:"ts'',move ts ts'' v \<Turnstile> \<^bold>\<not>(@e (safe e))" by blast
          ...
          hence dc:"ts',move ts ts' v \<Turnstile>  (@e \<^bold>\<not>\<^bold>\<langle>re(c) \<^bold>\<and> re(e)\<^bold>\<rangle> ) \<^bold>\<rightarrow> ( \<^bold>\<box>\<^bold>\<tau> @e \<^bold>\<not>\<^bold>\<langle>re(c) \<^bold>\<and> re(e)\<^bold>\<rangle>)" using not_eq_v
            by simp
          hence no_coll_after_evol:"ts',move ts ts' v \<Turnstile> ( \<^bold>\<box>\<^bold>\<tau> @e \<^bold>\<not>\<^bold>\<langle>re(c) \<^bold>\<and> re(e)\<^bold>\<rangle>)" using safe' 
            by simp
          hence 1:"ts'',move ts' ts'' (move ts ts' v) \<Turnstile> @e \<^bold>\<not> \<^bold>\<langle>re(c) \<^bold>\<and> re(e)\<^bold>\<rangle>" using evolve.hyps by simp
          have move_eq:"move ts' ts'' (move ts ts' v) = move ts ts'' v" using "evolve.hyps" 
              abstract.evolve abstract.refl move_trans by blast
          from 1 have "ts'', move ts ts'' v \<Turnstile> @e \<^bold>\<not>\<^bold>\<langle>re(c) \<^bold>\<and> re(e)\<^bold>\<rangle>" using move_eq by fastforce
      ...
      next
        ...
        proof (rule ccontr)
          ...
          from this have " (ts',v' \<Turnstile> re(c) \<^bold>\<and> re(e))"
            using n_def backwards_wdr_res_stab by blast 
          hence "ts',ve \<Turnstile> \<^bold>\<not> safe (e)" using c_neq_e c_def v'_def somewhere_leq by meson
          thus False using wd_res.hyps move_stab ve_def by fastforce                                     
        qed
      next
        case (wd_clm ts' ts'')
        have "move ts ts' v = move ts' ts'' (move ts ts' v)" using move_stability_wdc wd_clm.hyps move_trans 
        ...
        proof (rule ccontr)
          ...
          from this have " (ts',v' \<Turnstile> re(c) \<^bold>\<and> re(e))"
            using d_def backwards_wdc_res_stab by blast 
          hence "ts',ve \<Turnstile> \<^bold>\<not> safe (e)" using c_neq_e c_def v'_def somewhere_leq by meson
          thus False using wd_clm.hyps move_stab ve_def by fastforce                                     
        qed
      next
        case (cr_res ts' ts'')
        have local_LC: "ts',(move ts ts' v) \<Turnstile> \<^bold>\<forall>d.( \<^bold>\<exists> c. ( (@c (pcc c d)) \<^bold>\<or> (@d (pcc c d)))) \<^bold>\<rightarrow> \<^bold>\<box>r(d) \<^bold>\<bottom>  " 
        ...
        proof (rule ccontr)
          ...
          proof
            ...
            proof
              ...
              thus False using assm' cr_res.hyps c_def move_stab ve_def by force
            next
              ...
              hence "ts',move ts ts' v \<Turnstile> @e (pcc c e)" using ve_def move_stab switch_unique by fastforce
              hence pcc:"ts',move ts ts' v \<Turnstile> (@c (pcc c e)) \<^bold>\<or> (@e (pcc c e))" by blast
              have "ts',move ts ts' v \<Turnstile>( \<^bold>\<exists> c.( (@c (pcc c e)) \<^bold>\<or> (@e (pcc c e)))) \<^bold>\<rightarrow> \<^bold>\<box>r(e) \<^bold>\<bottom>  " 
          ...
          next
            ...
            proof
              ...
              from this obtain v' where v'_def:"(v' \<le> ve) \<and> (ts'',v' \<Turnstile> re(c) \<^bold>\<and> re(e))" by blast
              with backwards_res_stab have overlap: "ts',v' \<Turnstile>   re(c) \<^bold>\<and> (re(e))"
                using c_def  backwards_res_stab c_neq_e neq2 by (metis (no_types, lifting) d_def neq)
              hence unsafe2:"ts',ve \<Turnstile>\<^bold>\<not> safe(e)" 
                using  c_neq_e somewhere_leq v'_def by blast
              from cr_res.hyps have "ts',move ts ts'' v \<Turnstile> @e (safe(e))" using move_stab by force
            ...
            next
              ...
              with backwards_res_act have "ts',v' \<Turnstile>   (re(c) \<^bold>\<or> cl(c)) \<^bold>\<and> (re(e) )"
                using c_def  backwards_res_stab c_neq_e 
                by (metis (no_types, lifting) d_def eq2)
              hence "\<exists>v'. (v' \<le> ve) \<and> (ts',v' \<Turnstile> (re(c) \<^bold>\<or> cl(c)) \<^bold>\<and> (re(e)))"  using v'_def by blast
              ...
              proof
                ...
                thus False using assm' cr_res.hyps c_def move_stab ve_def by fastforce
              next
                ...
                hence "ts',move ts ts' v \<Turnstile> @e (pcc e c)" using ve_def move_stab  switch_unique by fastforce
                hence pcc:"ts', move ts ts' v \<Turnstile> (@e (pcc e c)) \<^bold>\<or> (@c (pcc e c))" by blast
                have "ts',move ts ts' v \<Turnstile>( \<^bold>\<exists> d. (@d (pcc d c)) \<^bold>\<or> (@c (pcc d c))) \<^bold>\<rightarrow> \<^bold>\<box>r(c) \<^bold>\<bottom>  " 

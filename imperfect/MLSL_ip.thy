...
proof (rule allI|rule notI)+
  fix ts v
  assume contra:"ts,v \<Turnstile> (re(c) \<^bold>\<smile> (\<^bold>\<omega> > 0) \<^bold>\<smile> re(c)) " 
...
proof (rule allI|rule notI)+
  fix ts v
  assume assm:"ts,v \<Turnstile> (cl(c) \<^bold>\<frown> \<^bold>\<top> \<^bold>\<frown> re(c))"
...
proof (rule allI|rule impI)+
  ...
  from chop and len_v1 len_v2 restr_v1 restr_v2 show "ts,v \<Turnstile>cl(c) \<^bold>\<frown> cl(c)"
    using hchop_def assm by (metis (no_types, lifting))
qed
...
proof (rule allI|rule impI)+
  ...
  from len_ge_0 and widt_one and no_car show "ts,v \<Turnstile>free" by blast      
qed
...
proof (rule allI|rule impI)+
  ...
  proof
    ...
    show  re':"ts',v \<Turnstile> re(c)" using len_eq ts'_def restrict re by auto
  next
    assume cl:"ts,v \<Turnstile>cl(c)"
    have len_eq:"len v ts c = len v ts' c" using ts'_def create_reservation_length_stable by blast
    have "clm ts c \<le> res ts' c"
      by (meson traffic.create_res_subseteq2 ts'_def)
    then have restrict:"restrict v (clm ts ) c \<sqsubseteq> restrict v (res ts' ) c" using traffic.create_res_subseteq2 ts'_def  less_eq_nat_int.rep_eq eq_iff 
      by (metis (no_types) cl inf.orderE inf_commute order_trans restriction.restrict_clm restriction.restrict_def)
...
lemma reservation2: "(\<Turnstile>(\<^bold>\<box>r(c) re(c)) \<^bold>\<rightarrow> (re(c) \<^bold>\<or> cl(c)))" 
  using mlsl_imperfect.backwards_res_act traffic.always_create_res by blast

...
proof
  ...
  have "res ts c \<sqsubseteq> res ts' c" using assm traffic.create_res_subseteq1  create_reservation_dict res_dict by auto
  hence restr_subs_res:"restrict v (res ts) c \<sqsubseteq> restrict v (res ts') c" using res_dict restrict_dict create_reservation_dict 
    using assm restriction.restrict_view by auto 
  have "clm ts c \<sqsubseteq> res ts' c" using assm traffic.create_res_subseteq2 using res_dict restrict_dict create_reservation_dict 
    using assm restriction.restrict_view clm_dict by auto 
  hence restr_subs_clm:"restrict v (clm ts) c \<sqsubseteq> restrict v (res ts') c" using  restrict_dict  
    using assm restriction.restrict_view by auto 
  ...
  proof
    ...
    hence "restrict v (res ts)c = lan v" using restriction.restrict_eq_lan_subs using restrict_one assm restrict_dict by auto
  ...
  next
    ...
    proof
      ...
      hence "restrict v (clm ts)c = lan v" using restriction.restrict_eq_lan_subs using restrict_one assm  restrict_dict by auto
      thus " ts,v \<Turnstile> (re(c) \<^bold>\<or> cl(c))" using assm len_eq by auto
    next
      ...
      hence False using restriction.create_reservation_restrict_union assm clm_empty restr_res_empty restrict_dict clm_dict res_dict union_dict
        using restriction_class.create_reservation_restrict_union by auto
      thus ?thesis by simp
    qed
  qed
qed
...
proof
  ...
  from assm have len_eq:"len v ts c = len v ts' c" using create_reservation_length_stable by blast
  have "res ts c = res ts' c" using assm traffic.create_res_subseteq1_neq res_dict create_reservation_dict by auto
  thus " ts,v \<Turnstile> (re(c))" using assm len_eq restriction.restrict_def restrict_dict by auto
qed
...
using create_claim_length_stable traffic.create_clm_eq_res res_dict create_claim_dict
...
using withdraw_claim_length_stable traffic.withdraw_clm_eq_res res_dict withdraw_claim_dict 
...
proof 
  ...
  proof (cases "c=d")
    case True
      then have wdr_conseq:"res ts' c = Abs_nat_int{n} \<and> n \<^bold>\<in> res ts c" using withdraw_reservation_def withdraw_reservation_dict 
        using assm res_dict by auto
      then have "lan v =  Abs_nat_int{n}" using assm res_dict restrict_dict Abs_nat_int_inverse  
      ...
      then show ?thesis using res_dict assm el_dict el_def card'_dict restrict_dict withdraw_reservation_length_stable restrict_def 
  ...
  next    
    case False
      then have "res ts c = res ts' c" using res_dict withdraw_reservation_def withdraw_reservation_dict assm by simp
      then show ?thesis using withdraw_reservation_length_stable withdraw_reservation_dict assm res_dict  restrict_dict 
        using restriction_class.restrict_def' by auto
  qed          
qed
...
proof (rule allI| rule impI)+ 
  ...
  proof
    ...
    have restrict:"restrict v (res ts ) c = restrict v (res ts' ) c" using restrict_dict res_dict create_reservation_dict
...
lemma reservation2: "(\<Turnstile>(\<^bold>\<box>r(c) re(c)) \<^bold>\<rightarrow> (re(c) \<^bold>\<or> cl(c)))" 
  using backwards_res_act traffic.always_create_res create_reservation_dict res_dict clm_dict restrict_dict 
  by metis

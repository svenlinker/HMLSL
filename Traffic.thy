...
proof -
 ...
 (* have non_e:"\<forall>c. (left (sp c) < right (sp c))" by (simp add: Abs_realInt_inverse sp_def)*)
...
proof
  ...
  proof
    ...
    hence suc_n_in_clm:"n+1 \<^bold>\<in> clm ts c" 
      by (metis UnE assm disj inf.absorb_iff2 inf_sup_absorb insert_subset n_def nat_int.el.rep_eq nat_int.inter_absorb1 nat_int.inter_sym less_eq_nat_int.rep_eq sup.cobounded2 sup_bot.right_neutral)
    have suc_n_not_in_res:"n+1 \<^bold>\<notin> res ts c" 
    proof -
      have "Rep_nat_int (res ts c) \<noteq> {n, n + 1}" 
        by (metis assm disj n_def nat_int.inter_absorb1 nat_int.inter_sym less_eq_nat_int.rep_eq sup.cobounded2)
      then show ?thesis
...
proof
  ...
  thus "clm ts c \<sqsubseteq> res ts' c" 
    by (metis Un_commute clm_consec_res disjoint nat_int.inter_subseteq1 nat_int.un_subset1 nat_int.un_subset2 nat_int.union_def)
qed
...
proof
  fix ts
  have "clm ts c = \<emptyset> \<or> clm ts c \<noteq> \<emptyset>" by blast
  then show " \<exists>ts'. (ts \<^bold>\<midarrow>r(c)\<^bold>\<rightarrow> ts')"
  proof 
    assume empty:"clm ts c = \<emptyset>"
    obtain ts' where ts'_def:"ts' = ts" by simp
    hence "ts  \<^bold>\<midarrow>r(c)\<^bold>\<rightarrow> ts'" 
      using create_reservation_def empty fun_upd_triv nat_int.un_empty_absorb1 by auto
    thus ?thesis ..
  next
    assume non_empty:"clm ts c \<noteq> \<emptyset>"
 ...
 (* have non_e:"(\<forall>c. left ((fst ts') c) < right ((fst ts') c))" by (simp add: space_nonempty ts'_def)*)
  have re_geq_one:"\<forall>c. |fst (snd ts') c| \<ge> 1" 
by (smt One_nat_def Suc_leI Un_commute add_is_0 atLeastOneRes nat_int.card_un_add clm_consec_res fst_conv fun_upd_apply nat_int.union_def neq0_conv non_empty snd_conv ts'_def)
  have re_leq_two:"\<forall>c. |(fst (snd ts')) c| \<le> 2" 
    by (metis (no_types, lifting) Un_commute add.commute atMostTwoLanes atMostTwoRes nat_int.card_un_add clm_consec_res fun_upd_apply nat_int.union_def non_empty prod.sel(1) prod.sel(2) ts'_def)
  ...
  proof
    fix d
    show "|((fst (snd ts')) d)| =2 \<longrightarrow> (\<exists>n . Rep_nat_int ((fst (snd ts')) d) = {n,n+1})"
    proof
      ...
      next
        ...
        thus "(\<exists>n . Rep_nat_int ((fst (snd ts')) d) = {n,n+1})"
        proof
          assume clm_empty:"clm ts c = \<emptyset>"
          hence "(fst (snd ts')) d = (res ts c)" using non_empty by blast
          thus ?thesis using clm_empty non_empty by blast
        next
          assume clm_nonempty:"clm ts c \<noteq> \<emptyset>"
          hence at_least_one_clm:"|clm ts c| \<ge> 1" using nat_int.card_non_empty_geq_one by blast
          from clm_nonempty have n1:"\<exists>n1. n1 \<^bold>\<in> clm ts c" by (simp add: nat_int.non_empty_elem_in)
          have n2:"\<exists>n2. n2 \<^bold>\<in> res ts c" 
            by (metis (no_types, lifting) clm_consec_res disjoint nat_int.consec_def nat_int.el.rep_eq nat_int.inter_absorb1 less_eq_nat_int.rep_eq non_empty subsetI)
          ...
          have consec:"\<exists> n. Rep_nat_int ((res ts) c) \<union> Rep_nat_int ((clm ts) c) = {n, n+1}" 
            using clmNextRes non_empty by auto
          ...
          proof 
            assume eq:"n1=n2+1"
            hence rep_un:"Rep_nat_int (clm ts c) \<union> Rep_nat_int (res ts c) = {n2,n2+1}"
              using rep_clm rep_res by auto
            hence "\<exists>n m . Rep_nat_int (clm ts c) \<union> Rep_nat_int (res ts c) = {n..m}"  
              by (metis One_nat_def add.right_neutral add_Suc_right atLeastAtMost_insertL atLeastAtMost_singleton_iff le_add1)
            hence  un_in_type:"Rep_nat_int (clm ts c) \<union> Rep_nat_int (res ts c)  \<in>  {S . (\<exists> (m::nat) n .{m..n }=S)}" by blast
            hence "Rep_nat_int (clm ts c \<squnion> res ts c) =  {n2,n2+1}" 
              by (simp add: Abs_nat_int_inverse rep_un nat_int.union_def)
            thus ?thesis 
              by (metis rep_un def_ts'_res eq doubleton_eq_iff n1_n2_un nat_int.union_def)
          next
            assume eq:"n2=n1+1"
            hence rep_un:"Rep_nat_int (clm ts c) \<union> Rep_nat_int (res ts c) = {n1,n1+1}"
              using rep_clm rep_res by auto
            hence "\<exists>n m . Rep_nat_int (clm ts c) \<union> Rep_nat_int (res ts c) = {n..m}"  
              by (metis One_nat_def add.right_neutral add_Suc_right atLeastAtMost_insertL atLeastAtMost_singleton_iff le_add1)
            hence  un_in_type:"Rep_nat_int (clm ts c) \<union> Rep_nat_int (res ts c)  \<in>  {S . (\<exists> (m::nat) n .  {m..n }=S) }" by blast
            hence "Rep_nat_int (clm ts c \<squnion> res ts c) =  {n1,n1+1}" 
              by (simp add: Abs_nat_int_inverse rep_un nat_int.union_def)
            thus ?thesis 
              by (metis rep_un def_ts'_res eq  n1_n2_un nat_int.union_def)
          qed
        qed
      qed
    qed
  ...
  (*                    (\<forall>c. left ((fst ts) c) < right ((fst ts) c)) \<and>*)
                      (\<forall>c. ((fst (snd ts))) c \<sqinter> ((fst (snd (snd ts)))) c = \<emptyset> )  \<and>
                      (\<forall>c. |(fst (snd ts)) c| \<ge> 1) \<and>
                      (\<forall>c. |(fst (snd ts)) c| \<le> 2) \<and>
                      (\<forall>c. |(fst (snd (snd ts)) c)| \<le> 1) \<and>
...
using clNextRe mem_Collect_eq dyn_geq_zero ps_ge_zero sd_ge_zero by blast
...
using withdraw_reservation_def nat_int.subseteq_refl less_eq_nat_int.rep_eq nat_int.el.rep_eq 

 ...
 \<lparr> ext = shift (ext (move ts ts' v)) (pos ts'' (own v) - pos ts' (own  v)),
  ...
  proof -
    have f2: "\<forall>x0 x1. (x1::real) + x0 = x0 + x1"
      by auto
    have "pos ts'' (own v) + - 1 * pos ts' (own v) + (pos ts' (own v) + - 1 * pos ts (own v)) = pos ts'' (own v) + - 1 * pos ts (own v)"
      by auto
    then have "real_int.shift (ext v) (pos ts'' (own v) + - 1 * pos ts (own v)) = real_int.shift (real_int.shift (ext v) (pos ts' (own v) + - 1 * pos ts (own v))) (pos ts'' (own v) + - 1 * pos ts' (own v))"
      using f2 real_int.shift_additivity  by (metis (no_types) \<open>\<And>y x r. real_int.shift r (x + y) = real_int.shift (real_int.shift r x) y\<close>)
    then show ?thesis

...
lemma len_sub_int:"len v ts c \<le> ext v" 
  using less_eq_real_int_def perfect_sensors.len_left perfect_sensors.len_right by blast
  ...
  next
    ...
    proof
      ...
      have len_geq:"left(len v1 ( ts) c) \<ge> left (ext v)"  
        using l2 perfect_sensors.len_left by auto
      show "left (len v1 ( ts) c) = left (ext v1)"
      proof (rule ccontr)
  ...
  next
    ...
    next
      ...
      then have len:"len v2 ( ts) c = Abs_real_int (left (ext v2),left (ext v2))"
        ...
        by (metis (no_types, hide_lams) l2 le_less_trans not_less order.asym perfect_sensors.space_nonempty r2)   
      show "(right ((len v2 ts) c) = right (ext v2))" 
      proof (cases "right ((space ts v) c) \<ge> left (ext v)")
        ...
        hence "len v (ts) c = Abs_real_int (left (ext v), left (ext v))" 
          ...
          by (meson less_trans not_less perfect_sensors.space_nonempty)
        show "(right ((len v2 ts) c) = right (ext v2))"  
        proof -
...
proof
  ...
  proof (cases " left ((space ts v) c) \<le> right (ext v)")
    ...
    hence outside_right1: "\<not> left ((space ts v) c) \<le> right (ext v1)" 
      using assm h_chop_middle2 left_v2 by fastforce
    from outside_right have outside_right2: "\<not> left ((space ts v) c) \<le> right (ext v2 )"
      using assm hchop_def real_int.rchop_def by auto
    show "(len v ( ts) c = ext v)" 
    proof -
      have f1: "left (ext v) = left (ext v1) \<and> right (ext v1) = left (ext v2) \<and> right (ext v2) = right (ext v) \<and> lan v = lan v1 \<and> lan v = lan v2 \<and> own v = own v1 \<and> own v = own v2"
        using assm hchop_def real_int.rchop_def by auto
      with assm have left:"left (ext v) = left (len v ts c)" using Abs_real_int_inverse 
        by (smt Abs_real_int_inverse len_def mem_Collect_eq outside_right2 prod.collapse prod.inject space_def left.rep_eq)
      from f1 and assm have right:"right (ext v) = right (len v ts c)" using len_def outside_right space_def by auto
      from left and right show "(len v ( ts) c = ext v)" 
        using Rep_real_int_inject prod_eqI by fastforce
    qed
  next
    ...
    proof (cases " right ((space ts v) c) \<ge> left (ext v)")
      ...
      hence outside_left1:"\<not>right ((space ts v) c) \<ge> left (ext v1)" 
        using assm hchop_def real_int.rchop_def by auto
      from outside_left have outside_left2:"\<not>right ((space ts v) c) \<ge> left (ext v2)"
        using assm h_chop_middle1 left_v2 by fastforce
      show "(len v ( ts) c = ext v)" 
      proof -
        have f1: "left (ext v) = left (ext v1) \<and> right (ext v1) = left (ext v2) \<and> right (ext v2) = right (ext v) \<and> lan v = lan v1 \<and> lan v = lan v2"
          using assm hchop_def real_int.rchop_def by auto
        with assm have left:"left (ext v) = left (len v ts c)" 
          using  len_def outside_left1 real_int.left_leq_right inside_right  Abs_real_int_inverse by auto
        from f1 and assm have right:"right (ext v) = right (len v ts c)" using len_def
          by (smt Abs_real_int_inverse hchop_def mem_Collect_eq outside_left prod.inject surjective_pairing space_def right.rep_eq left.rep_eq)
        from left and right show "(len v ( ts) c = ext v)" 
          using Rep_real_int_inject prod_eqI by fastforce
      qed
    next
      assume inside_left:"right ((space ts v) c) \<ge> left (ext v)"
      have len_def_in:"len v ( ts) c = Abs_real_int ((max (left (ext v)) (left ((space ts v) c))), 
                              min (right (ext v)) (right ((space ts v) c)))" 
        using   inside_left inside_right len_def by auto                               
      have max_left:"max (left (ext v)) (left ((space ts v) c)) = max (left (ext v1)) (left ((space ts v) c))"
        using assm hchop_def real_int.rchop_def by auto
      have v1_eq:"left(ext v1) = left (ext v)" 
        using assm hchop_def real_int.rchop_def by auto
      hence left_len:"left (ext v) = left (len v1 ( ts) c)" 
        using left_v1 by auto 
      have min_right:" min (right (ext v)) (right ((space ts v) c)) =  min (right (ext v2)) (right ((space ts v) c))" 
        using assm hchop_def real_int.rchop_def by auto
      have v2_eq:"right(ext v2) = right (ext v)" 
        using assm hchop_def real_int.rchop_def by auto
      hence right_len:"right (ext v) = right (len v2 ( ts) c)" 
        using right_v2 by auto 
      from inside_left and inside_right have len_in_type:"((max (left (ext v)) (left ((space ts v) c)), min (right (ext v)) (right ((space ts v) c)))) 
            \<in> {r :: real*real . fst r \<le> snd r}" 
        using CollectD CollectI Rep_real_int fst_conv snd_conv by auto
      hence len_in_type12:"((max (left (ext v1)) (left ((space ts v) c)), min (right (ext v2)) (right ((space ts v) c)))) 
            \<in> {r :: real*real . fst r \<le> snd r}" 
        using max_left min_right by auto
      show "(len v ( ts) c) = ext v"
      proof (rule ccontr)
        assume contra:"len v ( ts) c \<noteq>  ext v"
        hence "left (len v ( ts) c) > left (ext v) \<or> right (len v ( ts) c) < right (ext v)" 
          using Abs_real_int_inverse 
          by (metis Rep_real_int_inverse left.rep_eq less_le perfect_sensors.len_left perfect_sensors.len_right prod.collapse right.rep_eq)
        thus "False"
        proof
          assume left_less: "left (ext v)  < left (len v ( ts) c)"
          have max_left_c:"max (left (ext v)) (left (( space ts v) c)) = left (( space ts v) c)"
            using Abs_real_int_inverse left_less len_def_in len_in_type by auto
          hence max_left1_c:"max (left (ext v1)) (left ((space ts v) c)) = left ((space ts v) c)"
            using max_left by auto 
          have left1_less:"left (ext v1) < left ((space ts v) c)" 
            using Abs_real_int_inverse left_less len_def_in len_in_type12 min_right v1_eq by auto
          show False
          proof (cases " left ((space ts v) c) \<le> right (ext v1)") 
            assume inside:" left ((space ts v) c) \<le> right (ext v1)"
            have len1_def_in:"len v1 ( ts) c = Abs_real_int ((max (left (ext v1)) (left ((space ts v) c))), 
                              min (right (ext v1)) (right ((space ts v) c)))" 
              using inside inside_left left_len left_v1 len_def assm hchop_def space_def by auto 
            hence len_in_type1:"((max (left (ext v1)) (left ((space ts v) c)), min (right (ext v1)) (right ((space ts v) c)))) 
            \<in> {r :: real*real . fst r \<le> snd r}" 
              using Rep_real_int inside max_left1_c by auto
            have "left (len v1 ( ts) c) \<noteq> left (ext v1)" 
              using Abs_real_int_inverse left1_less len1_def_in len_in_type1 by auto         
            thus False 
              using left_v1 by blast
          next
            assume outside:" \<not>left ((space ts v) c) \<le> right (ext v1)"
            hence in_v2:"left((space ts v) c) > left (ext v2)" 
              using assm right_v1 by auto
            have left_in_v2:"left ((space ts v) c) \<le> right (ext v2)" 
              using inside_right v2_eq by auto
            have in_v2':"right ((space ts v) c) \<ge> left (ext v2)" 
              by (meson in_v2 less_le order_trans real_int.left_leq_right)
            have len2_def_in:"len v2 ( ts) c = Abs_real_int ((max (left (ext v2)) (left ((space ts v) c))), 
                              min (right (ext v2)) (right ((space ts v) c)))" 
              using assm hchop_def in_v2 len_def len_in_type min_right 
              using inside_right left_v2 v2_eq space_def by auto
            hence len_in_type2:"((max (left (ext v2)) (left ((space ts v) c)), min (right (ext v2)) (right ((space ts v) c)))) 
            \<in> {r :: real*real . fst r \<le> snd r}" 
              using in_v2 len_in_type min_right by auto
            have "left (len v2 ( ts) c) \<noteq> left (ext v2)" 
              using Abs_real_int_inverse left1_less len2_def_in len_in_type2 in_v2   assm hchop_def 
              by (smt fst_eqD left.rep_eq)
            thus False
              by (simp add: assm)  
          qed
        next
          assume right_greater:"right (len v ( ts) c) < right (ext v)"
          have min_right_c:"min (right (ext v)) (right ((space ts v) c)) = right ((space ts v) c)"
            using Abs_real_int_inverse right_greater len_def_in len_in_type by auto
          hence min_right2_c:"min (right (ext v2)) (right ((space ts v) c)) = right ((space ts v) c)"
            using  min_right by auto  
          have right2_greater:"right (ext v2) > right ((space ts v) c)" 
            using Abs_real_int_inverse right_greater len_def_in len_in_type12 min_right v2_eq
            by (metis len_in_type min_right2_c old.prod.inject prod.collapse right.rep_eq)
          show False
          proof (cases " left (ext v2) \<le> right ((space ts v) c) ")
            assume inside:" left (ext v2) \<le> right ((space ts v) c)"
            have inside_v2:"left ((space ts v) c) \<le> right (ext v2)" 
              using inside_right v2_eq by auto
            have len2_def_in:"len v2 ( ts) c = Abs_real_int ((max (left (ext v2)) (left ((space ts v) c))), 
                              min (right (ext v2)) (right ((space ts v) c)))" 
              using assm hchop_def inside_v2 len_def len_in_type min_right 
                inside space_def by auto
            hence len_in_type2:"((max (left (ext v2)) (left ((space ts v) c)), min (right (ext v2)) (right ((space ts v) c)))) 
            \<in> {r :: real*real . fst r \<le> snd r}" 
              using fst_conv inside less_le min_right_c perfect_sensors.space_nonempty v2_eq by fastforce
            have "right (len v2 ( ts) c) \<noteq> right (ext v2)"
              using Abs_real_int_inverse len2_def_in len_in_type2 right2_greater by auto
            thus False using right_v2 by blast
          next
            assume outside:" \<not>left (ext v2) \<le> right ((space ts v) c)"
            hence in_v1_r:"right (ext v1) > right ((space ts v) c)" 
              using assm right_v1 by auto
            hence left_in_v1:"left((space ts v) c) \<le> right (ext v1)" 
              using less_trans not_less real_int.left_leq_right by blast
                
            have right_in_v1:"right((space ts v) c) \<ge> left (ext v1)" 
              using inside_left v1_eq by auto 
            have len1_def_in:"len v1 ( ts) c = Abs_real_int ((max (left (ext v1)) (left ((space ts v) c))), 
                              min (right (ext v1)) (right ((space ts v) c)))" 
              using left_in_v1 len_def right_in_v1 assm hchop_def in_v1_r inside_left  space_nonempty v1_eq space_def   
              by (simp  )
            hence len_in_type1:"((max (left (ext v1)) (left ((space ts v) c)), min (right (ext v1)) (right ((space ts v) c)))) 
            \<in> {r :: real*real . fst r \<le> snd r}" 
              using in_v1_r len_in_type12 by auto
            have "right (len v1 ( ts) c) \<noteq> right (ext v1)"
              using Abs_real_int_inverse in_v1_r len1_def_in len_in_type12 by auto
  ...
  next
    ...
    next 
      ...
      then have "max (left (ext v2)) (left ((space ts v2) c)) \<ge> min (right (ext v2)) (right ((space ts v2) c))"
        using assm hchop_def perfect_sensors.space_def by auto
      then have "max (left (ext v2)) (left ((space ts v2) c)) = min (right (ext v2)) (right ((space ts v2) c))"
        using Abs_real_int_inverse 
        by (metis (no_types, hide_lams) antisym_conv assm hchop_def len_v_empty max_def min.bounded_iff not_le perfect_sensors.space_def right_inside_v2 right_len_leq view.h_chop_middle2)
      thus "\<parallel>len v2 ( ts) c\<parallel> = 0" using Abs_real_int_inverse 
        by (metis (no_types, hide_lams) assm hchop_def len_v len_v2 len_v_empty perfect_sensors.space_def right_len_leq)
    qed
  qed
qed
    ...
    next 
      ...
      proof (cases "right (len v ( ts) c) < right (ext v1)")
        ...
        have inside_right_v1:"\<not>left ((space ts v) c) > right (ext v1)" 
          by (meson inside_right less_trans min_less_iff_disj min_less_v1 order.asym perfect_sensors.space_nonempty)
      ...
      next
        ...
        proof (cases "left (len v ( ts) c) > left (ext v2)")
          ...
          have inside_left_v2:"  \<not>left (ext v2) > right ((space ts v) c) "
            by (meson inside_left less_max_iff_disj less_trans max_geq_v1 order.asym perfect_sensors.space_nonempty)
    ...
    next
      ...
      next
        ...
        thus False 
          ...
          by (smt CollectI fst_conv snd_conv left.rep_eq right.rep_eq)
      qed
    qed
  qed
qed
...
proof
  ...
  proof -
    ...
    have inside'': "left ((space ts v') c) < right (ext v') \<and> right ((space ts v') c) > left (ext v')"
      using space_eq assm by (metis (no_types, lifting)  inside')
...
lemma ext_eq_len_eq:"ext v = ext v'  \<longrightarrow> len v ( ts) c = len v' ( ts) c"
proof
...
lemma switch_length_stable:"(v=c>v') \<longrightarrow> len v ts c = len v' ts c"
  using ext_eq_len_eq switch_def by blast
end
end
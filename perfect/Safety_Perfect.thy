  ...
  next
    ...
    proof (rule)
      ...
      proof
        ...
        hence "\<exists>v'. (v' \<le> move ts ts'' v) \<and> (ts'',v' \<Turnstile> re(c) \<^bold>\<and> re(e))" using view.somewhere_leq   
          by meson
        ...
        hence "ts',move ts ts'' v \<Turnstile>\<^bold>\<langle> re(c) \<^bold>\<and> (re(e) \<^bold>\<or> cl(e)) \<^bold>\<rangle>" using view.somewhere_leq by meson
      ...
      next 
        ...
        next
          ...
          hence "ts',move ts ts'' v \<Turnstile>\<^bold>\<langle> (re(c) \<^bold>\<or> cl(c)) \<^bold>\<and> (re(e) ) \<^bold>\<rangle>" using view.somewhere_leq by meson
...
proof (rule allI|rule impI)+
  fix ts v v' 
  fix e d :: cars
  assume assm: "ts,v \<Turnstile>  \<^bold>\<forall>e. safe(e)" and v'_def:"(v=c>v')" and nequals:"ts,v \<Turnstile> \<^bold>\<not>(d \<^bold>=  e)"

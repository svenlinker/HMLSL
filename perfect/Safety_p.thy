...
proof (rule allI|rule impI)+
  fix ts v ts' e
  assume assm:"ts,v \<Turnstile> ( \<^bold>\<forall>e. safe e ) \<^bold>\<and> DC \<^bold>\<and> LC" and abs:"ts \<^bold>\<Rightarrow> ts'"
...
proof (rule allI|rule impI)+
  fix ts v v' e
  assume assm: "ts,v \<Turnstile>  \<^bold>\<forall>e. safe(e)" and v'_def:"(v=c>v')"
  show "ts,v' \<Turnstile>  safe(e)"
  proof(rule ccontr) 
    assume "\<not> ( ts,v' \<Turnstile>  ( safe(e)))"
    then have e_def:"ts,v' \<Turnstile> \<^bold>\<not> safe(e)" by blast
    ...
    then show False using assm by blast
  qed
qed

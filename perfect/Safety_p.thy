...
proof (rule allI|rule impI)+
    ...
    proof (rule)
      ...
      proof
        ...
        next
          ...
          hence "ts',move ts ts'' v \<Turnstile> \<^bold>\<not> (c \<^bold>=e) \<^bold>\<and> \<^bold>\<langle> re(c) \<^bold>\<and> cl(e)\<^bold>\<rangle>" using e_def and neg by force
      ...
      next 
        ...
        proof
          ...
          hence unsafe2:"ts',move ts ts'' v \<Turnstile> \<^bold>\<langle>re(c) \<^bold>\<and> re(e)\<^bold>\<rangle>" 
            using  neg somewhere_leq v'_def by blast
        ...
        next
          ...
          next
            ...
            hence "ts',move ts ts'' v \<Turnstile> \<^bold>\<not> (c \<^bold>=e) \<^bold>\<and> \<^bold>\<langle> cl(c) \<^bold>\<and> re(e)\<^bold>\<rangle>" using e_def neg by blast
  ...
  next 
    ...
    proof (rule)
      ...
      then have "ts', move ts ts'' v \<Turnstile> \<^bold>\<langle>re(c) \<^bold>\<and> re(e)\<^bold>\<rangle>" using v'_def somewhere_leq by meson   
      thus False using cr_clm.hyps move_stab e_def neg by fastforce 
    qed 
  next
    case (wd_res ts' ts'')
...
proof (rule allI|rule impI)+
  fix ts v v' 
  fix e d :: cars
  assume assm: "ts,v \<Turnstile>  \<^bold>\<forall>e. safe(e)" and v'_def:"(v=c>v')" and neg:"ts,v \<Turnstile> \<^bold>\<not>(d \<^bold>=  e)"
  ...
  proof(rule) 
    ...
    from e_def obtain v'sub where v'sub_def:"(v'sub \<le> v') \<and> (ts,v'sub \<Turnstile> re(d) \<^bold>\<and> re(e))" 
      using somewhere_leq by fastforce 
    ...
    then show False using assm neg by blast
  qed
qed
